...
# ── Dataset ───────────────────────────────────────────────────
export NUM_SHARDS="${NUM_SHARDS:-20}"                   # total shard count (split even/odd)
export IMAGES_PER_SHARD="${IMAGES_PER_SHARD:-100}"      # image–label pairs per shard
...
export WD_CACHE_DIR="${WD_CACHE_DIR:-/tmp/warpdrive-cache}"
